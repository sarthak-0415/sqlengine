					...
					for(i=current_page;i<NUM_PAGES;i++)
					{
						if (str_size <= (PAGESIZE - page[i].data_size) )
						{
							if(page[i].data_size == 0){
								...
								//the tablenum_pagenum map is int->string. the existing string is read into p1. 
								//we modify the string to include the new page number and add the new string to the map
								pageNum_list p1;
	...
	string selectCommand(string query)
	{
		...
		*/
		...
		if(columns[0] != "*")
		{
			...
			delimiter = "&";
			while(result.find(delimiter)!=string::npos)
			{
				delimiter = "&";
				result_row = result.substr(0, result.find(delimiter));
			  	result = result.substr(result.find(delimiter) + 1, result.length() );
				
				string delimiter2;
				delimiter2 = "#";
				select_results.append(result_row.substr(0, result_row.find(delimiter2) +1));
				result_row = result_row.substr(result_row.find(delimiter2)+1, result_row.length());
				vector<string> result_row_tokens=split(result_row, ',');
				
				for( vector<int>::iterator itr = att_nos.begin() ; itr != att_nos.end() ; ++itr)
				{
					select_results.append(result_row_tokens[(*itr)-1]);
			...
			delimiter = "#";
			select_results.append(result.substr(0, result.find(delimiter) +1));
			result = result_row.substr(result_row.find(delimiter)+1, result_row.length());
			vector<string> result_tokens=split(result, ',');
				
			for( vector<int>::iterator itr = att_nos.begin() ; itr != att_nos.end() ; ++itr)
			{
				select_results.append(result_tokens[(*itr)-1]);
				select_results.append(",");
			}
	...
	while(1)
	{
		...
		switch(choice)
		{
			...
			case 6:
				...
				break;
		  	case 7:
		  		nothing  = object.get_record_from_table("countries");
		  		cout <<"sjkdafalisdg ilfa"<< nothing<<endl;
				break;
			case 8:
